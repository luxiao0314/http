 ...
 */
package com.plugin.gradle.lucio.core.khttp.responses

import com.plugin.gradle.lucio.core.khttp.extensions.getSuperclasses
import com.plugin.gradle.lucio.core.khttp.extensions.split
import com.plugin.gradle.lucio.core.khttp.extensions.splitLines
import com.plugin.gradle.lucio.core.khttp.requests.GenericRequest
import com.plugin.gradle.lucio.core.khttp.requests.Request
import com.plugin.gradle.lucio.core.khttp.structures.cookie.Cookie
import com.plugin.gradle.lucio.core.khttp.structures.cookie.CookieJar
import com.plugin.gradle.lucio.core.khttp.structures.maps.CaseInsensitiveMap
...
class GenericResponse internal constructor(override val request: Request) : Response {
    ...
    internal companion object {
        ...
        internal val HttpURLConnection.cookieJar: CookieJar
            get() = CookieJar(
                *this.headerFields.filter { it.key == "Set-Cookie" }.flatMap { it.value }.filter(String::isNotEmpty).map(
                    ::Cookie
                ).toTypedArray()
            )
        ...
        get() {
            if (this._connection == null) {
                this._connection = URL(this.request.url).openRedirectingConnection(
                    ...
                    (defaultStartInitializers + this@GenericResponse.initializers + defaultEndInitializers).forEach {
                        it(
    ...
    override var encoding: Charset
        get() {
            ...
            this.headers["Content-Type"]?.let {
                val charset = it.split(";").map { it.split("=") }.filter { it[0].trim().toLowerCase() == "charset" }
